...
{{% game-info %}}
 ...
 <<사이버펑크 2077>>(Cyberpunk 2077)의 주인공 V는 게임 초반에 시한부 선고를 받는다. 머릿속에 박힌 칩이 V의 의식을 서서히 잠식해 가고, 남은 시간은 그리 길지 않다. 하지만 정작 플레이어는 서두를 이유가 없다. 메인 퀘스트를 미뤄 두고 나이트 시티 곳곳을 누비며 서브 퀘스트를 하더라도 V의 상태는 조금도 나빠지지 않기 때문이다. 서사가 강조되는 오픈월드 게임에서 이처럼 서사와 게임 플레이가 어긋나는 경우는 드물지 않다.

 <<블러드 오브 던워커>>(The Blood of Dawnwalker)는 이런 어긋남을 극복하려는 시도가 돋보이는 게임이다. [<<더 위쳐 3: 와일드 헌트>>](/review/witcher3/)(The Witcher 3: Wild Hunt)의 게임 디렉터였던 콘라드 토마슈키에비치(Konrad Tomaszkiewicz)가 설립한 레벨 울브즈의 첫 작품으로, 가족이 제물로 바쳐지기까지 남은 30일이라는 기한을 게임 속 시간 시스템으로 구현했다. 이로써 이야기 속 기한은 게임 플레이의 제약이 되고, 퀘스트와 활동 하나하나가 한정된 시간을 어디에 쓸 것인가에 대한 선택이 된다. 서사 속의 시간 제약을 게임 플레이로 끌어들여 플레이어의 선택에 무게를 더하려 한 것이다.

 서사를 시스템과 결부시키면 게임의 경험은 어떻게 달라질까. 이 글에서는 시간을 자원으로 삼은 이 과감한 시도가 게임 경험 전체를 어떻게 좌우했는지, 그리고 어떤 부분에서 아쉬움을 남겼는지를 살펴보려 한다.

## 스토리

> 수세기 동안 그림자 속에 숨어 있던 뱀파이어들이 마침내 모습을 드러내어 자유와 권력을 쟁취하려고 합니다. 그 뒤를 이어 또 다른 밤의 존재들이 세상에 나타납니다. 전설은 현실이 되고, 역사는 완전히 새로 써지게 됩니다.
>
> 게임의 주인공 코엔은 영원토록 낮의 세계와 밤의 왕국 사이를 넘나드는 존재인 '던워커'가 된 청년입니다. 인류를 위해 싸우거나, 저주받은 힘을 받아들이고 가족을 구하세요. "네 영혼은 네가 사랑하는 자들의 목숨만큼 가치가 있는가?" 어떤 길을 고르든 이 질문은 계속 코엔을 따라다닐 것입니다.[^official-story]
...
#### 개발진의 의도
 ...
 가장 그럴듯한 것은 이 게임의 규칙을 몸으로 익히게 하려는 의도다. 스킬 포인트는 퀘스트보다 훨씬 자주 쓰게 되는 만큼, 의미 있는 행동에는 시간이 든다는 규칙을 가장 자주 체감하게 만드는 수단이기도 하다. 한편 퀘스트 하나를 할지, 그 시간에 강해질지를 저울질하게 만들려는 의도였을 수도 있다. 전투와 이동에는 시간이 들지 않아 경험치는 얼마든지 쌓을 수 있으니, 이를 실제 힘으로 바꾸는 순간에 비용을 걸어 지나치게 강해지는 것을 막으려 했다는 해석도 가능하다. 다만 그런 의도였다면, 경험치나 스킬 효과로 조절할 성장 밸런스에 서사를 진행할 시간까지 요구했다는 점은 아쉽다.
 ...
 어떤 의도였든, 결과적으로 이 구조가 만들어낸 플레이 경험은 검술이 아닌 스킬에 포인트를 넣기가 꺼려진다는 것이다. 물론 커뮤니티에서는 검술이 아닌 다른 빌드를 선택한 사례도 종종 찾아볼 수 있다. 다만 처음 플레이하는 입장에서는 남은 시간이 얼마나 넉넉한지, 다른 능력에 투자해도 충분히 싸울 수 있을지 알기 어려웠고, 낮과 밤 모두 쓸 수 있는 검술이 더 안전한 선택으로 느껴졌다.
...
#### 같은 조작, 다른 체감
 ...
 1인칭인 킹덤 컴 시리즈에서는 카메라가 바라보는 방향이 곧 캐릭터가 바라보는 방향이다. 그래서 검의 방향을 조작하는 동안에도, 던워커에서처럼 카메라를 따로 돌리지 못한다는 답답함은 덜했다. 반면 3인칭인 던워커에서 오른쪽 스틱은 캐릭터와 별개로 카메라를 돌리는 역할을 한다. 캐릭터가 적을 마주하고 있는 동안에도 카메라를 돌려 주변을 살필 수 있다는 것이 3인칭의 장점인데, 여기에 검의 방향까지 묶어버리니 전투 중에는 이 장점을 살릴 수가 없다.
...
![여러 적을 상대하는 코엔](./images/BloodOfDawnwalker05.webp)
 ...
 그럼에도 방향 전투가 주는 재미는 분명하다. 무작정 칼을 휘두르기보다 상대의 자세와 모션을 읽고 빈틈을 노려야 하며, 패링과 반격도 그만큼 더 짜릿해진다. 여러 적에게 둘러싸이면 적마다 방어하는 방향이 달라 공격하기가 어려워지는데, 이 역시 방향 전투만이 줄 수 있는 긴장감이다. 던워커에서도 이런 재미는 분명 느낄 수 있었다. 다만 같은 조작이라도 어떤 시점의 게임에 들어가느냐에 따라 치러야 하는 대가가 달라진다. 킹덤 컴 시리즈에서는 비교적 자연스럽게 받아들였던 조작이지만, 던워커에서는 주변을 살피는 데 필요한 카메라 조작과 충돌한다.
...
![포 아너(좌), 마운트 앤 블레이드 2: 배너로드(우)](./images/BloodOfDawnwalker06.webp)
 ...
 어느 쪽도 완벽한 답은 아니지만, 던워커에서도 참고할 만한 방식들이다. 시야 바깥의 적이 공격하지 않게 하면 보이지 않는 공격에 당할 일은 줄어들지만, 주변을 확인하기 어렵다는 불편은 남고 이 규칙을 알아차린 뒤에는 전투의 긴장감도 줄어든다. 상대의 자세를 읽고 빈틈을 노리는 재미를 살리면서도 주변 상황을 확인할 수 있도록, 조작이나 화면 표시를 보완했으면 어땠을까.
...
![몸에 문양을 새긴 코엔. 주술은 상처로 그린 문양을 통해 발휘된다.](./images/BloodOfDawnwalker07.webp)
 ...
 전투에서 여러 능력을 활용하는 데에는 장착 제한도 걸림돌이 된다. 액티브 능력을 장착할 수 있는 슬롯이 제한되어 있고, 심지어 이 슬롯을 차지하는 패시브 능력도 있기 때문이다.
...
> | **퀘스트** | ★★★★★(10점) | 위쳐 3 제작진다운 |
> | **성장** | ★★★★(8점) | 강해지는 체감은 확실, 초반의 시간 부담은 아쉬움 |
> | **전투** | ★★★☆(7점) | 자세를 읽고 반격하는 재미, 후반에는 줄어드는 긴장감 |
...
[^official-story]: [PlayStation 한국 공식 소개](https://www.playstation.com/ko-kr/games/the-blood-of-dawnwalker/)
